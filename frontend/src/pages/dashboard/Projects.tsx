...
import { Search, MoreHorizontal, CheckSquare, Plus, Loader2, X } from 'lucide-react';
...
export default function Projects() {
  ...
  const filtered = fuzzySearch(projects, search, (p) => [
    p.title || '',
    p.description || '',
    p.status || '',
  ]);
    ...
    <div className="space-y-6 max-w-[1400px] mx-auto w-full">
      ...
      <div className="relative w-full sm:max-w-sm">
        ...
        <Input 
          ...
          className="pl-9 pr-8 bg-background h-9 border-border/50 text-sm" 
          value={search} 
              ...
              <Card className="hover:shadow-md hover:border-border transition-all duration-300 h-full flex flex-col group cursor-pointer border-border/50 bg-card overflow-hidden relative">
                ...
                <CardContent className="p-5 flex-1 flex flex-col">
                  <div className="flex items-start justify-between mb-4">
                    <div className="flex items-center gap-3">
                      ...
                      <h3 className="font-semibold text-sm group-hover:text-primary transition-colors leading-tight">
                        {highlightMatches(project.title, search).map((seg, si) => 
                  ...
                  <p className="text-xs text-muted-foreground line-clamp-2 mb-6 flex-1 leading-relaxed">
                    {project.description ? highlightMatches(project.description, search).map((seg, si) => 
                      seg.highlight ? <mark key={si} className="bg-primary/20 text-primary rounded-sm px-0.5">{seg.text}</mark> : <span key={si}>{seg.text}</span>
                    ) : 'No description'}
                  </p>
